...
awk '{print $4}' "$GRCh38_CDS_genes" | uniq | grep -Ff -  "$T2T_CDS_genes" | sort -k1,1V -k2,2n > "$T2T_CDS_GeneID_Overlapping"
awk '{print $4}' "$T2T_CDS_genes"  | uniq | grep -Ff -  "$GRCh38_CDS_genes" | sort -k1,1V -k2,2n > "$GRCh38_CDS_GeneID_Overlapping"
...
# categorical file.
bedtools intersect -a "$T2T_CDS_GeneID_Overlapping" -b "$T2T_filtered_categorical_file" | sort -k1,1V -k2,2n | uniq > "$T2T_intersected_CDS"
bedtools intersect -a "$GRCh38_CDS_GeneID_Overlapping" -b "$GRCh38_filtered_categorical_file" | sort -k1,1V -k2,2n | uniq > "$GRCh38_intersected_CDS"
